...
public class IRPrinter implements Operators.Visitor<String> {
    ...
    String print(Operators expr) {
     ...
     @Override
    public String singleOp(Token op, Token first) {
        return parenthesize(op.lexeme, first);
    }

    @Override
    public String inspectBinaryOp(Token op, Token first, Token second) {
        return parenthesize(op.lexeme, first, second);
    }

    @Override
    public String inspectTernaryOperation(Token op, Token first, Token second, Token third) {
        return parenthesize(op.lexeme, first, second, third);
    }

    @Override
    public String inspectNullOperation(Token op) {
